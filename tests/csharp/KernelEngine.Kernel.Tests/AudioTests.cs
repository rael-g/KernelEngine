...
public class AudioTests
{
    ...
    private static unsafe ke_result MockLoadSound(ke_audio* self, sbyte* path, uint* outId)
    {
    ...
    [UnmanagedCallersOnly(CallConvs = new[] { typeof(System.Runtime.CompilerServices.CallConvCdecl) })]
    private static unsafe ke_result MockPlay(ke_audio* self, uint soundId, float volume, byte loop)
    {
        LastSoundId = soundId;
        LastVolume = volume;
        LastLoop = loop;
    ...
    public unsafe void LoadSound_ReturnsNone_WhenNativeFails()
    {
        var native = CreateMockNative();
        var audio = new Audio(native);
        LastResult = ke_result.KE_ERROR_IO;
