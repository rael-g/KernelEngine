...
void MeshSystem::Update(void* handle, ke_world* world, float dt, ke_frame_packet* packet)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        if (tc)
        {
            // Thread-safe recording into the packet
            // Using atomic increment on draw_count to allow parallel mesh systems in the future
...
{
    // Context must be persistent. In a real engine, we'd use an internal allocator or the world's.
    // For simplicity, we allocate once.
