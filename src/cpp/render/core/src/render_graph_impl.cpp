    ...
    : renderer_(renderer)
{
    ...
    api_.import_texture = [](ke_render_graph* self, const char* name, ke_texture_handle handle, ke_error** out_error) {
        (void)out_error;
        if (!self || !self->handle) return KE_ERROR;
        return static_cast<RenderGraphImpl*>(self->handle)->ImportTexture(name, handle);
    };
    api_.add_pass = [](ke_render_graph* self, const ke_render_pass_params* params, ke_error** out_error) {
        (void)out_error;
        if (!self || !self->handle) return KE_ERROR;
        return static_cast<RenderGraphImpl*>(self->handle)->AddPass(params);
    };
    api_.remove_pass = [](ke_render_graph* self, const char* name, ke_error** out_error) {
        (void)out_error;
        if (!self || !self->handle) return KE_ERROR;
        return static_cast<RenderGraphImpl*>(self->handle)->RemovePass(name);
    };
    api_.compile = [](ke_render_graph* self, ke_error** out_error) {
        (void)out_error;
        if (!self || !self->handle) return KE_ERROR;
        return static_cast<RenderGraphImpl*>(self->handle)->Compile();
    };
    api_.execute = [](ke_render_graph* self, const ke_frame_packet* packet, ke_error** out_error) {
        (void)out_error;
        if (!self || !self->handle) return KE_ERROR;
...
ke_result RenderGraphImpl::AddPass(const ke_render_pass_params* params)
{
    ...
    for (const auto& p : passes_) {
        if (p.name == params->name) return KE_ERROR;
    }
    ...
    for (const auto& pass : passes_) {
        for (const auto& r : pass.reads) {
            if (resources_.find(r.name) == resources_.end())
                return KE_RENDER_LOG_ERR(renderer_->GetContext().logger, KE_ERROR,
                    "RenderGraph.Compile",
...
ke_result RenderGraphImpl::EnsureResourceMaterialized(Resource& r)
{
    ...
    if (r.desc.type == KE_RESOURCE_TYPE_STORAGE_BUFFER) {
        ...
        return r.storage_buffer != kGpuInvalidHandle ? KE_OK : KE_ERROR;
    }
