...
static bool EncodeFallbackWhite(ke_logger* logger, ke_texture_data* out_data)
{
    log_warn(logger, "Failed to load texture; using white fallback");
    out_data->pixels = (uint8_t*)ke_alloc(4, 4);
    if (!out_data->pixels) return false;
...
static bool CopyToAllocator(const uint8_t* raw, int w, int h, ke_texture_data* out_data)
{
...
bool DecodeExternal(const std::string& path, ke_logger* logger, ke_texture_data* out_data)
{
    ...
    bool ok = CopyToAllocator(raw, w, h, out_data);
    stbi_image_free(raw); // matched with stbi_load (stb's CRT)
    return ok;
}

bool DecodeEmbedded(const aiTexture* et, ke_logger* logger, ke_texture_data* out_data)
{
    if (et->mHeight == 0)
    {
