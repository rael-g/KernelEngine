...
static ke_result EncodeFallbackWhite(ke_logger* logger, ke_texture_data* out_data)
{
    log_warn(logger, "Failed to load texture; using white fallback");
    out_data->pixels = (uint8_t*)ke_alloc(4, 4);
...
static ke_result CopyToAllocator(const uint8_t* raw, int w, int h, ke_texture_data* out_data)
{
    size_t byte_count = (size_t)w * h * 4;
    out_data->pixels = (uint8_t*)ke_alloc(byte_count, 4);
    if (!out_data->pixels) return KE_ERROR;
...
ke_result DecodeExternal(const std::string& path, ke_logger* logger, ke_texture_data* out_data)
{
    int w = 0, h = 0, ch = 0;
    uint8_t* raw = stbi_load(path.c_str(), &w, &h, &ch, 4);
    if (!raw) return EncodeFallbackWhite(logger, out_data);

    copy_string(out_data->path, sizeof(out_data->path), path.c_str());
    auto res = CopyToAllocator(raw, w, h, out_data);
    stbi_image_free(raw); // matched with stbi_load (stb's CRT)
    return res;
}

ke_result DecodeEmbedded(const aiTexture* et, ke_logger* logger, ke_texture_data* out_data)
{
    if (et->mHeight == 0)
    {
