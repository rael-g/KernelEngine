#pragma once

#include <kernel_engine/common/export.h>
#include <kernel_engine/task_scheduler/task_scheduler.h>

#ifdef __cplusplus
extern "C" {
#endif

#ifndef KE_TASK_SCHEDULER_API
    #ifdef KE_TASK_SCHEDULER_STATIC
        #define KE_TASK_SCHEDULER_API
    #else
        #ifdef KE_TASK_SCHEDULER_EXPORT
            #define KE_TASK_SCHEDULER_API KE_EXPORT
        #else
            #define KE_TASK_SCHEDULER_API KE_IMPORT
        #endif
    #endif
#endif

/**
 * @brief Creates a task scheduler implementation based on enkiTS.
 */
KE_TASK_SCHEDULER_API ke_result ke_task_scheduler_enki_create(struct ke_allocator *allocator, struct ke_task_scheduler **out_scheduler, struct ke_error **out_error);

#ifdef __cplusplus
}
#endif
