...
/// <summary>
/// Reflection-built <see cref="IInputActionMap{TEnum}"/> backed by a TOML document.
/// Loaded once at startup; immutable afterward.
/// </summary>
public sealed class InputActionMap<TEnum> : IInputActionMap<TEnum> where TEnum : struct, Enum
{
    ...
    {
        if (!File.Exists(path))
            throw new FileNotFoundException($"Input actions file not found: {path}", path);
    ...
    public static InputActionMap<TEnum> LoadFromText(string toml)
    {
        ...
        {
            if (!Enum.TryParse<TEnum>(actionName, ignoreCase: true, out var enumValue)) continue;
