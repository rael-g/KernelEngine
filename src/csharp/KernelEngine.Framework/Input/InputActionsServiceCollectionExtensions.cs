...
{
    /// <summary>
    /// Registers an <see cref="IInputActionMap{TEnum}"/> singleton built from
    /// the TOML file at <paramref name="path"/> (defaults to <c>actions.input</c>
    /// next to the executable). The map is loaded eagerly so missing-file or
    /// parse errors surface at startup rather than at the first input read.
    /// </summary>
