...
| 10 | Math: no engine-owned math library; users use their language's library directly (System.Numerics in C#, GLM in C++). Source AND target conventions are declared at runtime; a boundary adapter reconciles the delta. | See ADR-10 below. Avoids a third, incomplete math lib; avoids per-op vtable cost; keeps the engine's only math responsibility as a backend-driven boundary conversion |

### ADR-10 — Math is the user's library; the engine builds matrices in the backend-declared NDC convention (2026-05-22)

**Context.** The kernel had a hand-rolled `math.h` (`static inline` `ke_mat4_*`) and the C# Framework had a hand-ported managed twin (`Framework/Internal/Mat4.cs`). Two copies of the same conventions (column-major, RH, Vulkan [0,1], `ke_mat4_mul == b·a`) kept in sync by hand — the drift class that produced the shadow lightVP bug. GLM is a vcpkg dependency but unused.

> **Correction (same day).** A first draft of this ADR proposed a generic runtime **matrix-convert adapter** `convert(ke_mat4, src, dst)`. That was the **wrong shape**: it reconciles a "source convention" that never exists, because **no foreign-convention matrix ever crosses the framework→backend boundary**. The analysis below replaces it. The lens is the project's freedom philosophy: assume open doors, future forks with other backends, and that the architectural shape must survive use cases we haven't met yet.

**Where the problem actually lives.** Game code hands the engine **convention-neutral data**: `Vector3` position, `Quaternion` rotation, scale, and camera `Fov`/`Near`/`Far` — none of which carry handedness/z-range. The **engine** builds the matrices: the *world matrix* in the C kernel (`ke_mat4_from_transform` in `world.c`), the *view/projection* in the Framework (`Mat4.cs`). So:
1. **Math-library freedom is already fully served, with zero new infra.** A dev computes gameplay math with System.Numerics / GLM / anything and passes neutral primitives; the engine doesn't care which lib produced a `Vector3`. Nothing to build here.
2. **The real freedom problem is engine matrix *builders* hardcoding one backend's clip convention.** `Mat4.Perspective`/`Ortho` assume Vulkan [0,1]. A fork with an OpenGL/D3D backend (z in [-1,1], Y flip) would get **wrong** projection/view matrices today.

**Decision.**
1. **No engine-owned math library.** Users use their language's standard lib directly (`System.Numerics` in C#, GLM in C++) for all gameplay math. Compile-time choice; inline + SIMD; no vtable. The boundary is neutral data, so this freedom needs no adapter.
2. **`ke_mat4` (column-major `float[16]`) is the wire format** crossing the ABI. Storage convention is fixed by this contract; a backend that internally prefers another layout transposes **inside the backend plugin**, not via a universal converter.
3. **The active backend declares its NDC/clip convention as a capability** — `ke_render.get_ndc_convention()` → `{ z_range (zero-to-one | minus-one-to-one), y_flip, handedness }` (bgfx reads `caps.homogeneousDepth` etc.).
4. **Engine matrix builders consume that capability** instead of hardcoding Vulkan. `Mat4.Perspective`/`Ortho`/`LookAt` (Framework) build directly in the active backend's convention → a forked backend gets correct matrices with no game-code change (the swap test, applied to math). There is **no** `src→dst` matrix converter — builders produce the target convention directly.
5. **Optional future advanced path:** if we ever let a dev supply a *raw* view/projection matrix (custom camera), *that input* carries a per-input convention tag; the engine adapts it. This is opt-in and per-input, not a global negotiation — out of scope until the API exposes raw-matrix input.

**Consequences.**
- Keep the `ke_*` ABI types. The kernel keeps its two *live, internal* C helpers — `ke_mat4_from_transform` + `ke_mat4_mul` (used by the C TransformSystem in `world.c`; C, so GLM can't apply) — these are internal world-matrix computation, not a user-facing math lib. The `mul == b·a` order is documented at that single site.
- **Dead code to delete:** `ke_mat4_identity`, `ke_mat4_inv`, `ke_mat4_proj`, `ke_mat4_lookat`, `ke_mat4_ortho` in `math.h` have **0 callers** (verified 2026-05-22) — the C# `Mat4.cs` reimplemented proj/lookat/ortho. Remove them.
- **`Mat4.cs` deleted; replaced by `Framework/Internal/ViewProjection.cs` + System.Numerics.** General math is System.Numerics (`Matrix4x4.Invert` for the camera view; `Vector3` cross/dot/normalize for the light view). `ViewProjection` is the **explicit convention adapter** and builds the only two convention-bound matrices the engine produces: the **projection** (in the backend's NDC depth-range/Y, via the capability) and the **view `LookAt`** (right-handed, forward = +(target−eye), which `Matrix4x4.CreateLookAt` does NOT match — it uses zaxis = eye−target).
- **Honest correction:** the original ADR draft framed this as "no adapter needed." That was wrong. The exercise (deleting `Mat4`, trying System.Numerics) proved a convention adapter is **irreducible**: System.Numerics' ortho depth-sign differs from ours, its `CreateLookAt` handedness differs, and no std lib targets arbitrary NDC. `ViewProjection` *is* that adapter (and `Mat4` was a disguised one). What materially changed vs the old `Mat4`: (1) the 5 dead C ops are gone; (2) projection is now driven by the backend-declared NDC convention instead of hardcoded Vulkan — the actual freedom infra a forked backend needs; (3) `InvertTrs` → `Matrix4x4.Invert`.
- **No `ke_math_convention` matrix-convert `convert(src,dst)`** — the convention is applied at *build* time inside `ViewProjection` (which knows the target), not as a post-hoc matrix-to-matrix conversion.
