...
### ADR-10 — Math is the user's library; the engine owns only a runtime convention adapter (2026-05-22)

**Context.** The kernel had a hand-rolled `math.h` (`static inline` `ke_mat4_*`) and the C# Framework had a hand-ported managed twin (`Framework/Internal/Mat4.cs`, `unsafe`). Two copies of the same conventions (column-major, RH, Vulkan [0,1], and the `ke_mat4_mul == b·a` order) that must be kept in sync by hand — the exact class of drift that produced the shadow lightVP bug. GLM is already a vcpkg dependency but unused.

**Decision.**
1. **No engine-owned math library.** Rolling our own is rejected (it would be a third, incomplete lib forcing users to mix two math libraries). Users use their language's standard library *directly and fully*: `System.Numerics` in C#, GLM in C++. Chosen at compile time; both inline + SIMD, zero vtable.
2. **The kernel defines only the wire type + a convention descriptor.** `ke_mat4` = `float[16]` (the only thing crossing the ABI) and `ke_math_convention { handedness, z_range, y_down, column_major }`.
3. **Both conventions are declared at runtime, not detected.** A matrix's handedness/z-range is *semantic*, not recoverable from its 16 floats — auto-detection is impossible. So: the matrix **producer** (e.g. the Framework, using System.Numerics) declares its **source** convention via config; the active render backend (`ke_render`) exposes its **target** convention as a capability (`get_math_convention()`, bgfx reads `caps.homogeneousDepth` etc.). Swap the backend → target changes automatically (the swap test, applied to math).
4. **A boundary adapter reconciles the delta.** `convert(in ke_mat4, src, dst)` — pure function of the two descriptors. Identity when `src == dst`.

**Consequences.**
- **Overhead is negligible and not new.** Heavy math (mul/invert, millions/frame) runs entirely inside System.Numerics/GLM, untouched by the adapter. The adapter only touches matrices that *cross the ABI* (camera + per-draw world, dozens/frame) — a branchless 16-float transform that the engine already performs today (the `ke_mat4` blit). `src`/`dst` are read once at init and cached; the per-matrix path is a predicted branch, never a vtable call.
- Deletes the hand-rolled `math.h` operations (keep the `ke_*` types) and `Mat4.cs`; the convention quirk (`mul == b·a`) is encapsulated once.
- **Not a prerequisite for removing `unsafe`** from the Framework — that only needs `Mat4.cs` rewritten without pointers (or replaced by System.Numerics). The full adapter is tracked as its own card; do it as a dedicated effort, not bundled into the unsafe cleanup.
