...
- **[UPSCALER-PREP] AI upscaler plugin (DLSS / FSR / XeSS via NVIDIA Streamline)** — wrap **NVIDIA Streamline** (https://github.com/NVIDIAGameWorks/Streamline, MIT, vendor-neutral despite the NVIDIA branding) as `KernelEngine.Upscaler.Streamline` plugin. Single API dispatches to DLSS/FSR/XeSS underneath; user picks at runtime. **Prerequisites met**: render-graph + clustered forward shading already shipped (see Done.md), so named slots for motion-vector / jittered-depth / linear-light output can be authored as new render-graph passes. **What remains**: research the Streamline integration shape (motion-vector encoding, jitter sequence wiring), prototype as a render-graph pass set, then ship the plugin. Not next-up — schedule when an actual consumer needs upscaling.
...
- **Status**: **Pillar 1 ✅ shipped** (3-thread split with one-directional snapshot exchange — see Done.md, 2026-06-08 audit). Pillars 2 (phantom `SimContext`/`RenderContext` tokens) + 3 (API-by-absence: sim cannot resolve `IRenderer`) + Roslyn analyzer are still pending.
