...
| A14 | **R2.5d polish — close the runtime branch cleanly** | Two follow-ups to land on `feat/runtime-v2` (or a successor branch) before merging the runtime split into `main`. Functional R2.5c-final shipped on the branch (parallel scheduler + waves + defer queue + fixed timestep + debug checks); these are doctrine/cosmetic finishings. ⟶ **(1) Strip flecs addons via vendored amalgamated.** Today vcpkg ships `flecs_static.lib` with FLECS_PIPELINE / FLECS_SYSTEM / FLECS_TIMER linked in even though we don't use them (Option D per RuntimeArchitectureV2.md §8). To physically strip: drop flecs's published amalgamated `flecs.h` + `flecs.c` into `extern/flecs/`, write a ~30-line CMake target that compiles it with `FLECS_CUSTOM_BUILD` + selected addons (`FLECS_QUERY`, `FLECS_OBSERVER`, etc.), replace the `find_package(flecs)` calls in `src/c/ecs/flecs/CMakeLists.txt`, drop the vcpkg flecs override. Cost: ~10K lines of vendored C (two files) + manual update on upstream releases. Benefit: doctrine matches reality, ~50KB binary saved, vcpkg simpler. ⟶ **(2) ClangSharp regen for the new C# surface.** Currently `src/csharp/KernelEngine.Runtime/NativeMethods.cs` and `src/csharp/KernelEngine.Ecs.Flecs/NativeMethods.cs` are hand-rolled P/Invoke. `src/csharp/KernelEngine.Kernel/Native/Generated/Kernel/ke_runtime_system_params.cs` carries a manual patch (header banner notes the divergence). Replace with autogen: write `Runtime.rsp` for `KernelEngine.Runtime` pointing at `kernel_engine/kernel/runtime/runtime_create.h` + `kernel_engine/kernel/runtime/system_ctx.h`; write `EcsFlecs.rsp` for `KernelEngine.Ecs.Flecs` pointing at `kernel_engine/kernel/world/ke_ecs_flecs.h`; update `KernelEngine.Kernel/Native/Kernel.rsp` to exclude the now-duplicated `ke_ecs_flecs_create` and `ke_runtime_create`; run `python scripts/generate_bindings.py`; delete hand-rolled `NativeMethods.cs` files; verify all tests stay green. | new — R2.5d polish list per RuntimeArchitectureV2.md §14 |
