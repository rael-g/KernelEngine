...
- **What**: For each `examples/csharp/0X_*/`, capture a reference frame (after a known-good run) → `examples/csharp/0X_*/golden.png`. In CI (or `python scripts/coverage.py`), run each example headless for N seconds, capture frame, compare against golden with pixel-tolerance threshold. Fail if diff > 2%.
