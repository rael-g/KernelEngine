...
| P19 | **CLI `ke compile shaders`** | Wrap `python scripts/compile_shaders.py` as a `ke` subverb so users don't shell out to python directly. Two MVP shapes: (a) thin wrapper that invokes the script with the right working dir (simplest, matches P14's `ke new game` precedent of "leverage existing tools"); (b) when F.BA Phase B lands, the script becomes multi-backend — the CLI verb should expose `ke compile shaders --backend dx11` or default to "all installed backends". Take option (b) once F.BA is in flight; until then (a) is the minimum-viable shipping today. | new |
| P20 | **User shaders / custom shader materials — Godot-like full UX** (scope confirmed 2026-05-30) | Today every mesh uses engine built-in `fs_basic`. Target: Godot-like ergonomics where dev writes processor functions (`vertex()`/`fragment()`/`light()`) and engine wraps with scaffolding (PBR + transforms + lighting + clipping). Confirmed scope: 3 incremental layers, all three eventually shipped. **Layer 1 — `PostEffectNode`** (1 session): plant node in scene → registers as F.RC2 `KE_PASS_FULLSCREEN` graph pass; dev writes full `.sc` shader. Closes Godot UX gap for screen-effect chains (AA → CRT → aberration). **Layer 2 — `Material.NextPass` chain** (1 session): material has optional `NextPass` ref to another material (Godot pattern, up to N levels); render system iterates the chain at `scene.opaque`, submits N draws per mesh. Combined with Layer 1 = "quad with stack of fullscreen-shader passes" exactly like Godot. **Layer 3 — shader preprocessor + scaffolding templates** (multi-session, the big chunk): define `.kshader` (or `.sc` with shader_type metadata); engine-side preprocessor reads `shader_type spatial`/`canvas_item`/`particles`/`sky`/`fog` + processor function bodies, injects them into the matching template (`spatial.template.sc`, etc.), emits final `.sc`, then bgfx shaderc emits per-backend `.bin`. Dev writes only hooks; engine handles PBR loop, transforms, clipping, cluster light integration. Changes the user base who can author shaders (no need to understand bgfx). **Sequence behind F.BA Phase B** (multi-backend shader compile must exist first) AND P19 (`ke build shaders` CLI integration). Touches: kernel material API (custom program + uniform table variant), render pipeline (Material.NextPass loop + custom-program bind), shader compile pipeline (preprocessor → shaderc per backend per shader), C# managed surface (`Material.FromShader(shader, uniforms)` + `PostEffectNode` + `ShaderResource`). | new |
...
##### [F.BA] Backend-agnostic for real (bgfx selectable + multi-backend shaders + agnostic-drift tests)
- **Tags**: `feat` (render), `infra` (tests), doctrine validation
- **Why**: The engine was *designed* backend-agnostic — `ke_render` C contract is pure (no bgfx symbols), `ViewProjection` adapts matrices via `ke_render.get_ndc_convention()`, every render code path goes through `GpuDevice`. But in practice today the only backend that actually runs is Vulkan, and three Vulkan-hardcoded spots block a swap. The user goal: prove the doctrine by running the same engine + same examples under Direct3D11 (and any other bgfx-supported backend) AND lock that in with regression tests so future features can't silently re-couple. "Toda hora testar com todos os backends é impossível" — testes têm que provar agnosticism, não revisão manual.
- **Architecturally agnostic today ✓**: ke_render contract pure, NDC adapter wired, shader-path resolution goes through `GpuDevice.GetShaderSubdir()` (already returns per-backend folder name), bgfx hides backend Y-flip in clip-space normalization (we report `y_flip=false` always to the engine and let bgfx adapt internally).
- **Vulkan-only today ✗** (audited 2026-05-30):
    1. `scripts/compile_shaders.py` hardcodes SPIR-V output (`-p spirv` + `compiled/spirv/` dir).
    2. `src/cpp/render/bgfx/src/bgfx_render_factory.cpp:24` defaults `renderer_type` sentinel to `bgfx::RendererType::Vulkan`; `BgfxRendererOptions` (C#) doesn't expose the field. ("currently → Vulkan; BgfxRendererOptions does not yet expose this" admitted in source.)
    3. `fs_basic.sc:79` hardcodes `coord.y = -coord.y * 0.5 + 0.5` on shadow UV. Comment at :102 confirms "Y-flipped target under Vulkan" — under D3D11 this flip is likely wrong (suspect, needs visual confirm post-swap). Other shaders (skybox, tonemap, ui_quad) haven't been audited.
- **Plan (4 phases, ~5h total):**
    - **Phase A — Selectable backend (~1-2h)**. Add `BgfxBackend` enum to bgfx plugin (`Auto | Vulkan | Direct3D11 | Direct3D12 | OpenGL | OpenGLES | Metal | Noop`); surface via `BgfxRendererOptions.Backend`; Project TOML key `[runtime.renderer] backend = "vulkan"` via existing `TomlOptionsBinder`. Native side maps 1-to-1 to `bgfx::RendererType::Enum`; `Auto` keeps current behavior. `Noop` is headless — needed for unit-test instantiation without a window.
    - **Phase B — Multi-backend shader build (~1h)**. Rewrite `compile_shaders.py` as a loop over `[("spirv", "vulkan"), ("dx11", "windows", "s_5_0"), ("glsl", "linux", "120"), ("metal", "osx", "metal")]`. Output per-backend folders (`shaders/compiled/{spirv,dx11,glsl,metal}/{name}.bin`). `GpuDevice.GetShaderSubdir()` already returns the right folder name per active backend; verify renderer reads from `${shader_path}/${subdir}/{name}.bin`.
    - **Phase C — Audit + fix shader hardcodes (~1h, biggest unknown)**. Grep all shaders for `coord.y = -`, `gl_FragCoord.y`, hardcoded depth ranges. Wrap backend-specific bits with `#ifdef BGFX_SHADER_LANGUAGE_HLSL` / `BGFX_SHADER_LANGUAGE_GLSL` macros where the math diverges. Re-test example 06 (shadow) + ex 09 (cluster lights) + ex 10 (bloom) + ex 14 (UI quad) under each backend.
    - **Phase D — Agnostic-drift tests (the load-bearing item)**:
        - **D.1 (structural, cheap, runs every build)**: xUnit test that greps `src/c/`, `src/csharp/KernelEngine.Kernel*`, `src/csharp/KernelEngine.Framework`, `src/csharp/KernelEngine.Kernel.Abstractions` for backend-specific symbols (`bgfx::`, `vulkan`, `d3d`, `glsl`, `Vulkan`, `Direct3D`). Match list must be empty — any hit fails. Catches PR-level drift before the costly visual test even runs.
        - **D.2 (visual parity, run-on-demand, GPU needed, MVP scope)**: 1 fixture = example 06 (shadow + directional light = exercises matrix conventions + shadow path = the things most likely to silently diverge). For each available backend (start with Vulkan + D3D11), render exactly 1 deterministic frame → bgfx readback of backbuffer → SHA-256 hash → compare to `tests/visual/baselines/06_{backend}.sha256`. Test fails on hash divergence. Updating baselines is an explicit commit ("re-bless the baseline"). Marked `[Trait("category", "visual")]` so it's opt-in (`dotnet test --filter Category=visual`) — doesn't slow normal builds.
        - Future expansion: more fixtures (cluster lights, bloom, UI), more backends (OpenGL via WARP on CI?). Out of scope for the MVP.
- **Acceptance**:
    - User can write `services.AddBgfxRenderer(opts => opts.Backend = BgfxBackend.Direct3D11)` OR set `[runtime.renderer] backend = "d3d11"` in Project TOML and example 06 renders identically (shadow visible, correct colors) under D3D11 as under Vulkan.
    - `dotnet test --filter Category=visual` runs Vulkan + D3D11 baselines and passes.
    - `dotnet test` (no filter) includes the structural agnostic-drift check and it passes (zero backend symbols in agnostic layers).
- **Effort**: M (~5h spread across A→B→C→D, but D.1 + skeleton of D.2 ideally land FIRST so phases A-C can be validated by tests as they go — per user's "D primeiro, depois A+B+C" preference).
- **Risk**: Phase C is the unknown — shader hardcodes may be more pervasive than the 1 known site. Visual regression on currently-working examples is the cost of being honest about agnosticism.
- **Why not now**: registered ahead of execution. Take when there's a focused session for it; doesn't gate any Tier P work.
