...
> **T0 (priority, do FIRST next session)**: rebuild `scripts/run_tests.py` and the entire coverage pipeline from scratch. The current state is a stack of patches over a fragile design — hardcoded module dict, manual gcovr-per-module loops, dedicated coverage build dir as a workaround for ninja not rebusting objects on flag flip, mixed C# + native pipelines in one ad-hoc script, no caching, no per-target precision, no CI integration. Throw it out and design a clean replacement (single tool or two clear stages, automatic module discovery, no shutil hacks).
...
| T8 | **NATIVE COVERAGE PIPELINE IS BROKEN** (discovered 2026-05-31, blocker for T2/T7). Running `scripts/run_tests.py` produces a report with ONLY 4 C# assemblies; ALL native modules (kernel + every plugin) are missing from `Summary.txt`, despite `cmake --preset win -DKE_COVERAGE=ON` + `enable_coverage()` on every relevant target. Earlier in the session a single run did show native modules (`src.c.kernel.*`, `src.cpp.render.*`, etc.) at non-zero coverage, but subsequent runs reproduce only C#. The `Cleanup` step (`cmake --preset win -DKE_COVERAGE=OFF`) at the end of the script is suspicious — it leaves the build cache configured WITHOUT coverage, so the next run's incremental build may keep stale non-instrumented objects (CMake might not re-trigger compilation when only `KE_COVERAGE` flips). Investigation needed: (a) does `KE_COVERAGE=ON` re-trigger object recompilation when objects already exist from a `KE_COVERAGE=OFF` build? (b) are `.gcno` files actually generated for `ke_kernel`/`ke_render_bgfx`/etc.? (c) does `gcovr` find them in `build/win/` paths? Provisional fix idea: skip the cleanup step (always leave coverage on) OR force a full rebuild when `KE_COVERAGE=ON` is requested. Also: `tests/cpp/CMakeLists.txt:95-97` calls `enable_coverage()` unconditionally (no `if(KE_COVERAGE)` guard) — harmless but inconsistent with the top-level guard pattern. **Until T8 is fixed, the "0% native coverage" claims elsewhere (e.g. T2) are unverified — the data is missing, not necessarily zero.** | blocker |
...
##### [F.BA] Backend-agnostic for real (bgfx selectable + multi-backend shaders + agnostic-drift tests) — ✅ DONE (2026-05-31, D.2 deferred)

- **Shipped (2026-05-31)**:
    - **D.1** ✅ — `AgnosticDriftTests.cs` in `KernelEngine.Kernel.Tests`: greps `src/c/kernel/`, `KernelEngine.Kernel`, `KernelEngine.Kernel.Abstractions`, `KernelEngine.Framework` for backend symbols (`bgfx::`, `Vulkan`, `Direct3D`, `d3d`, `glsl`, `vulkan`) on non-comment lines. Fails the build on any leak. Runs every `dotnet test`.
    - **A** ✅ — `BgfxBackend` enum (`Auto | Noop | Direct3D11 | Direct3D12 | Metal | OpenGLES | OpenGL | Vulkan`) maps 1-to-1 to `bgfx::RendererType::Enum`. `BgfxRendererOptions.Backend` (default `Auto`). TOML `[runtime.renderer] backend = "direct3d11"` works via standard .NET enum binding. `renderer_type = (uint)opts.Backend` replaces the hardcoded sentinel.
    - **B** ✅ — `compile_shaders.py` rewritten: loops over 6 backends (`spirv`, `dx11`, `dx12`, `glsl`, `essl`, `metal`), outputs to `compiled/{subdir}/`. `--backend spirv,dx11` filter. Incremental by mtime. `sys.exit(1)` on failure (CI-safe). Confirmed: 20/20 shaders compile for both `spirv` and `dx11`.
    - **C** ✅ — Full shader audit. Confirmed no backend-specific code changes needed: `u_lightVP` is always built with `y_flip=false` (bgfx reports this for all backends), so the shadow UV flip and fullscreen UV flip are correct for all backends. Misleading "Vulkan-specific" comments corrected in `fs_basic.sc` and `vs_fullscreen.sc`. One open item documented in-shader: cluster Y flip under OpenGL needs D.2 visual confirm.
- **Pending**:
    - **D.2** ⏳ — Visual parity test (SHA-256 per backend, opt-in `[Trait("category","visual")]`). Deferred: requires `Renderer.DumpFramebuffer()` (B4.3) and a D3D11 GPU fixture. Take when B4.3 is done and there's a session for visual validation. The cluster Y-flip under OpenGL is the one remaining shader unknown.
