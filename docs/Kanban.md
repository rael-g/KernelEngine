...
##### [F.RC2.Resume] Render-graph Phase 4.2+ — continuation checkpoint (2026-05-29)
- **Branch**: `feat/render-graph` — 14 commits ahead of main. Phases 1, 2, 3 (6 passes migrated), 4.1 (compute infra in executor) done.
- **Next session entry point**: implement `ClusteredForward::SetupClustered / UpdateClusterBounds / DispatchLightCull` (all empty stubs today). The shader `cs_light_cull.sc` is already written and expects 7 typed compute buffers (3 RO vec4: clusterBounds + pointLights + spotLights; 4 RW uint: pointIndices, pointCount, spotIndices, spotCount) plus uniforms u_clusterParams (numX, numY, numZ, maxLightsPerCluster), u_clusterParams2 (pointCount, spotCount), u_computeView. Grid is 16×8×24 per the shader's header comment. Dispatch is one workgroup per cluster (NUM_THREADS(1,1,1) so `dispatch(16,8,24)` = 3072 wgs).
- **Storage buffer materialization in the executor (Phase 4.1)** ships a generic uint32 read/write buffer (flag combo `kFmt32x1 | kTypeUint | kCompRW = 0x0327`). The 7 cluster buffers each need different flag combinations (RO vs RW, format 32x4 for vec4 vs 32x1 for uint, type FLOAT vs UINT) — either extend `ke_resource_desc` with format/access hints the executor reads, or have `ClusteredForward` create them outside the graph for now and reference them via raw `GpuDynamicIndexBufferHandle`. The latter is the path of least resistance for the first cut.
- **Compute program loading gap**: `CoreRenderer::LoadShader` + the `load_extra` lambda in SetupShader handle vs+fs pairs only (via `CreateProgram(vs, fs, true)`). Compute needs `CreateComputeProgram(cs, true)`. Add a `load_compute(name, &out_p)` helper inline in SetupShader before 4.2 calls SetupClustered.
- **Frustum slicing**: standard exponential Z partition (Olsson 2012). Per-cluster AABB built in view space from the camera projection's near/far; updated when projection changes (`bounds_dirty_` already exists in the class). Upload via `gpu->UpdateDynamicIndexBuffer(b_clusterBounds, 0, mem)` per dirty flip.
- **Phase 4.3 follow-up**: replace fs_basic.sc's brute-force loops over `u_pointLights[128]`/`u_spotLights[192]` (capped at 64 point / 48 spot, OBS at large counts) with cluster lookup reading b_pointLightIndices + b_pointLightCount keyed by `gl_FragCoord` → cluster index.
- **Phase 4.4 validation**: example 09 (~200 point lights) should render every light correctly — no silent dropouts past the 64 cap. Memory's "brute-force tomou o lugar" stops being true here.
