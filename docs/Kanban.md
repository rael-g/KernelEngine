...
- **[UPSCALER-PREP] AI upscaler plugin (DLSS / FSR / XeSS via NVIDIA Streamline)** — wrap **NVIDIA Streamline** (https://github.com/NVIDIAGameWorks/Streamline, MIT, vendor-neutral despite the NVIDIA branding) as `KernelEngine.Upscaler.Streamline` plugin. Single API dispatches to DLSS/FSR/XeSS underneath; user picks at runtime. **Hard prerequisite**: render-graph (`[F.RC2]`) must expose named slots for *motion vectors per draw*, *jittered depth*, and *linear-light pre-tonemap output* — without these passes, integration becomes hardcoded into `core_renderer.cpp` and fights the architecture. **Status (2026-06-05)**: `[F.RC2]` may already be shipped (see top-of-file audit note); re-evaluate after the audit sweep. Until then, treat as "research the Streamline integration shape; do nothing in source".
