...
- **Vestige to retire alongside step 3**: `src/cpp/render/core/src/view_ids.hpp` (enum Shadow/Scene/...). The "real" passes still hardcode `Id(ViewId::Scene)` etc. when calling `gpu->Submit`; once the executor owns view-id assignment, that enum has nothing to do.
- **Acceptance**: `SubmitPacketLegacy` deleted, `FrameSubmitter.cpp` either deleted or shrunk to a utility header, no caller references `view_ids.hpp`. All examples (01-15 + Pong) render identically before/after each sub-split.
- **Effort**: M (3 commits × ~1h each, the validation hops being the slow part). **Strictly after Phase 5 lands and the branch merges** — splitting the monolith while the C# wrapper is in flight multiplies the surface to revalidate.
- **Risk**: getting uniforms-to-buffers wrong is the same class of bug we hit at clustered shading bring-up (compute-write buffer rule, descriptor binding ordering). Use the GpuFatal incident's plan template as a model.
