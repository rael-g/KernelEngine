...
| P20 | **User shaders / custom shader materials** | Today every mesh uses the engine's built-in `fs_basic` PBR shader. User wants Godot-style `ShaderMaterial`: a material whose pixel/vertex programs come from the project (`res://shaders/water.sc`), with uniform/texture bindings declared by the dev. Touches: kernel material API (currently just `ke_material { r,g,b,a, texture, metallic, roughness, normal_map }` — needs a "custom program + uniform table" variant), render pipeline (must bind the right program per draw + skip the cluster-light loop or let custom shader opt-in), shader compile pipeline (user shaders compile alongside engine shaders, output to project-local dir), C# managed surface (`Material.FromShader(shader, uniforms)`). Notes: F.RC2 already lets a dev add a *pass*; this is the per-object analog. Strongly inspired by Godot's ShaderMaterial. **Substantial — 1-2 sessions of focused work; sequence behind F.BA so the shader compile pipeline is already multi-backend.** | new |
