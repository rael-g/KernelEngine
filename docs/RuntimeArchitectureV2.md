...
## 16. Sim/render pipelining — component snapshot, then its real substrate

Locked at design level 2026-06-10. **Implemented** on `refactor/ecs-memory-safety` (2026-07-10 through 2026-07-12) — §16.9 is the as-built record; §16.1–§16.8 below are kept as the original design log (including the parts that turned out wrong, §16.9 says which and why) so the reasoning isn't re-litigated from scratch. This section records WHY we picked the shape, so implementation doesn't re-litigate.
...
### 16.9 As built — what §16.3's mechanism got wrong, and what actually shipped

§16.3.1 claimed the unlock was free: double-buffer a component, let the wave-builder see `live`/`snap` as different keys, and sim-WRITE/render-READ fall into the *same wave* with no lock, no copy, no second world. That mechanism does not work, for a reason the design pass didn't have visibility into: **`ecs_readonly_begin`/`ecs_readonly_end` (flecs's mechanism for letting a wave's parallel bodies touch storage) is a single non-reentrant flag on the whole world**, not a stackable/scoped region. Two independent readonly regions — one for a sim wave, one for a render wave — cannot overlap on one flecs world without one of them prematurely flipping the world back to mutable while the other's threads still assume readonly guarantees. This is a hard blocker discovered empirically while trying to build the actual overlap (not a hypothetical): §16.3.1's "same wave" framing is retracted.

**What shipped instead — three sequential slices, each correcting the previous one's substrate:**

1. **First attempt (retracted): shadow entities inside the same flecs world.** Every entity owning a double-buffered component got a second, dedicated "shadow" entity (linked via a hidden component, cascade-deleted with the live entity) holding its `_snap` components — solving the *narrower* problem that came before this one (an unrelated structural change to the live entity was relocating its `_snap` column, corrupting an already-resolved render read; RuntimeArchitectureV2.md's own history has three prior corruption fixes in this exact area). This made the snapshot memory-safe against archetype churn, but render passes still called `ecs_query_resolve`/`ecs_get_id` against the shared flecs world — still hitting the readonly-mode blocker above the moment tick() tried to actually overlap sim and render. Retracted in full, including its `ke_ecs` API surface.

2. **The real substrate: extract render-phase queries into runtime-owned buffers ("R4").** At the sim→render boundary, `runtime_extract_render_state` resolves every render-phase system's queries against the live ECS **once**, merge-copies each query's matched columns into flat buffers `registered_system` itself owns (not flecs entities, not flecs components — plain `ke_alloc`'d arrays), and points that system's `ke_system_ctx_view` segments at them. From that instant on, a render-phase system makes **zero calls into `ke_ecs`** for the rest of the tick. This is the Bevy "extract" step, but the render side is a dumb, ownerless copy — not a second live ECS world, and not a fixed-schema packet (§16.2's option C, still correctly rejected: there is no schema, the extract is driven by each system's own declared queries).
3. **`ke_ecs` contract cleared of the pipelining surface entirely.** Since ke_ecs is never touched during a wave (sim-phase bodies read live-resolved segments; render-phase bodies read extracted buffers — both plain memory, no ke_ecs call from either), `concurrent_reads` (the readonly-mode wrapper) and the whole snapshot vtable (`component_register_v3`, `set_double_buffered`, `snapshot_cid`, `snapshot_entity`, `swap_snapshots`) were deleted from `ke_ecs.h` — six functions, none load-bearing once the extract owns the render side. `ke_ecs` is thread-safe by construction: nothing calls it concurrently, so it needs no lock, no readonly mode, no guard of its own. `ke_system_ctx_get`/`get_mut` (the entity-keyed read path `reads_snapshot` used to route) were deleted too — they had no real callers; every render pass already read exclusively through `ke_system_ctx_view`.

**The actual pipelining mechanism — async `tick()`, not a wave-builder trick.** `tick()` runs this tick's sim phases (PRE_UPDATE → FIXED_UPDATE → UPDATE → POST_UPDATE) on the calling thread, **joins the previous tick's render phase** (waits for the coordinator task the previous `tick()` call dispatched and did not wait for), extracts this tick's render state, then **dispatches this tick's render phase as one scheduler task and returns without waiting for it**. The overlap is: this tick's sim phases (step 1) run on the calling thread while the *previous* tick's render phase is still executing on a worker — genuine sim N+1 ‖ render N, achieved by deferring the join, not by two systems sharing a wave. The join must happen before this tick's own extract (extraction overwrites the very buffers a still-running render coordinator might be reading) — this ordering constraint, not the wave-builder, is what makes it correct.

**Constraint this design imposes going forward, not previously true:** a render-phase system's `defer_flush` (spawn/attach/detach/despawn, applied at that phase's wave barrier) now runs on the coordinator's worker thread, potentially concurrently with the *next* tick's sim-phase `defer_flush` calls into `ke_ecs` (since the join happens after sim runs, not before). No render pass performs structural ECS mutation today (every one reads via `ke_system_ctx_view` only, writes GPU resources through the named-resource table) — but the API does not forbid it, and if one ever did, that mutation would race. Documented here so it is not rediscovered as a surprise; not yet enforced by an assertion (project rule 6 forbids `assert`/`abort`, and propagating this as a `ke_error` would need defer_flush to carry error state it doesn't today).

**Measured (2026-07-12, `09_many_lights`, 10,000 point lights, clustered forward):** ~19 FPS (~52 ms/frame) synchronous → ~30 FPS (~33 ms/frame) with async dispatch. A ~55-60% frame-rate improvement on this scene, ahead of §16.1's original "~15-30% of CPU budget unused" estimate — plausibly because the per-frame cull + extract cost at 10k lights is large enough to make the overlap window worth more than the original estimate assumed.

**Shutdown crash — root-caused and fixed.** The async render dispatch initially produced a wgpu/Vulkan panic ("Trying to destroy a SurfaceSemaphores that is still in use by a SurfaceTexture", "Texture ... has been destroyed") on process shutdown across several examples — reproducible on close, not observed during steady-state running. The initial hypothesis (a present-thread affinity requirement on the render coordinator's worker, mirroring the macOS constraint in `RenderArchitectureV2.md` §9.7) was **wrong** and is retracted: a reproducer that ticks a handful of frames and then immediately unloads (no window/GUI involved) reproduced the crash reliably regardless of which worker the coordinator ran on. The real cause is a lifecycle-ordering bug: an example's loop breaks the instant `window.ShouldClose()` flips true, and the tick that just ran dispatched this tick's render phase **without waiting for it** (that's the whole point of the async design, §16.9 above). The very next thing the loop does is call `runtime.UnloadModules`, which tears down the GPU device and swapchain surface — while that last tick's render coordinator could still be recording/presenting on a worker thread. Nothing before this fix ever joined that in-flight task before module teardown; `runtime_destroy`'s join (added alongside the async dispatch) fires too late, since `UnloadModules` runs before the `IServiceProvider` disposes the `Runtime` wrapper. Fix: added `flush_render` to the `ke_runtime` vtable (blocks on any pending render coordinator, no-op if none), and `RuntimeStartup.UnloadModules` now calls it before invoking any module's `OnUnload`. Verified via the same isolated reproducer (5/5 clean runs post-fix, vs. reliably reproducing pre-fix) plus the full native (277/277) and managed (133/133) suites.
