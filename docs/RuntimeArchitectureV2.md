...
**Known open issue, not yet root-caused:** the async render dispatch has produced a wgpu/Vulkan panic ("Trying to destroy a SurfaceSemaphores that is still in use by a SurfaceTexture", "Texture ... has been destroyed") on process shutdown across several examples — reproducible on close, not observed during steady-state running. Leading hypothesis: the render coordinator task now runs the whole `begin_frame`→`end_frame` sequence (including the swapchain surface acquire/present) on whichever pool worker `ke_scheduler->dispatch` load-balances it to, rather than always the thread that created the `ke_gpu_device`/window surface — §9.7/§13.8 of `RenderArchitectureV2.md` already flagged a present-thread affinity requirement for macOS specifically; this suggests Windows/Vulkan via wgpu-native has a similar constraint the "everything else unpins" framing missed. Not yet fixed; tracked as the immediate next investigation, likely resolved by pinning the render coordinator task to a single consistent thread (`ke_scheduler->dispatch_pinned`) rather than load-balancing it per tick.
