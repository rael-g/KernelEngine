...
3. **World ownership model**: world OWNS ecs + runtime + scene_tree (transferred from host on `ke_world_create`). World BORROWS allocator + task_scheduler + logger (process-wide primitives). `world->destroy(world)` cascades scene_tree → runtime → ecs in reverse-create order. Host destroys scheduler + allocator afterwards.
...
- **D phases** — managed wrappers replace the R6-era managed Framework implementations. Order from memory: D1 World+SceneTree wrappers, D2 SceneLoader, D3 InputActions, ~~D4 render system wrappers — folded into the render plugin reading components directly post-R6~~, D5 cache/asset wrappers.
