...
### 9.4 Sim/render pipelining — merge-blocking scope of this branch (done, one open issue)
...
**Open issue before this can be called done:** a wgpu/Vulkan panic on process shutdown ("Trying to destroy a SurfaceSemaphores that is still in use by a SurfaceTexture") reproduces across several examples once the render phase runs on a load-balanced pool worker instead of always the calling thread — see §16.9's last paragraph. Not yet root-caused or fixed; the leading hypothesis is a present-thread affinity requirement (§9.7 already flagged this for macOS specifically; Windows/Vulkan via wgpu-native appears to need it too). Do not treat async dispatch as merge-ready until this is resolved.
...
- **NOT confirmed, and now falsified for the naive form: whole-phase-on-a-worker (surface acquire/present) thread affinity.** Dispatching the *entire* render phase (§9.4/§16.9 — `begin_frame` through `end_frame`, including the swapchain `get_current_texture`/present) as one task on a load-balanced pool worker produces a reproducible wgpu/Vulkan panic on shutdown ("Trying to destroy a SurfaceSemaphores that is still in use by a SurfaceTexture"). This is a *different* claim than encoder recording (which is fine per the bullet above) — it is specifically about the surface/swapchain lifecycle needing consistent thread affinity across frames, which "everything else unpins" (below) did not anticipate outside macOS. Unresolved; see `RuntimeArchitectureV2.md` §16.9's last paragraph.
...
**Revisited (2026-07-12) — the "everything else unpins" caveat's scope was too narrow.** Building the actual §16.9 async `tick()` surfaced a present-thread-affinity failure on **Windows/Vulkan via wgpu-native**, not just macOS. The recording-side claim in this paragraph stands (confirmed empirically, bullet above); the *surface acquire/present* side needs the same "pin to one consistent thread" treatment this paragraph reserved for macOS only. Fix path (not yet implemented): pin the render-phase coordinator task to a single fixed thread via `ke_scheduler->dispatch_pinned` — likely whichever thread created `ke_gpu_device` — rather than load-balancing it per tick.
...
- [x] Validate wgpu-native thread-safety (§13.8) for parallel command recording — confirmed (§9.7). **Not yet done: surface acquire/present thread affinity** — a real, reproducible shutdown crash under async dispatch (§9.4/§9.7, `RuntimeArchitectureV2.md` §16.9); do not unpin the render coordinator by default until fixed. `frames_in_flight` depth (§13.9) still unpicked.
