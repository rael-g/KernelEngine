...
**Status (2026-07-05): real opt-in threaded for shadow AND IBL.** `ke_render_feature_params` (`enable_shadows`, `enable_ibl`; NULL = both on, prior behavior) now drives whether each feature's GPU resource, pass, access-list dependency, and set-0 bindings exist at all — a module with `enable_shadows=0` declares no `shadow_map` resource, registers no `render.shadow` system, allocates no shadow bindings (frame_bgl skips 3-6), and links `FullyLit` instead of `ShadowMapVisibility`; same shape for IBL at bindings 7-8 (`NoIndirect` vs `IndirectIBL`). Verified: all four shadow×ibl permutations run in `09_many_lights`; `06_shadow_map` unchanged. The 2×2 is covered by four **hand-authored** material wrappers (`mat_test_flat` / `_no_shadow` / `_no_ibl` / `_no_shadow_no_ibl`) — the manual stand-in for the Mechanism-2 variant axis (§6.2.1). Cluster/light opt-in was intentionally **not** added as a third hand-authored axis: it would double the matrix to 8, which is the point at which the generator must exist first. The manual matrix stops here; the next step on this feature is Mechanism 2 (generate the variant permutations the game declares), not more `_no_x` files.
